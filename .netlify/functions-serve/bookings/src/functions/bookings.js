...
var import_http_errors = __toModule(require_http_errors());
...
// functions/supabase/supabase.ts
var import_supabase_js = __toModule(require_main6());
var supabase = (0, import_supabase_js.createClient)(process.env.SUPABASE_PROJECT_URL, process.env.SUPABASE_PUBLIC_ANON_KEY);
...
var handler = async (event, context) => {
  ...
  try {
    let { data: booking, error } = await supabase.from("booking").select("booking_id, created_at, product (*)");
    if (error) {
      throw (0, import_http_errors.default)(400, error.message, user);
    }
    return {
      statusCode: 200,
      body: JSON.stringify({ booking, error, user })
    };
  } catch (error) {
